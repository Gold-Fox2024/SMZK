...
::: tip 带权重的 TOPSIS
在实际问题中，各个指标的重要性往往不同。设权重向量 $w=(w_1,\ldots,w_m)$，满足 $w_j\ge 0,\ \sum_{j=1}^m w_j=1$。先构造加权标准化矩阵：
$$
v_{ij}=w_j z_{ij}
$$
再用 $V=(v_{ij})$ 来确定理想解并计算距离：
$$
V^+=(\max_i v_{i1},\ldots,\max_i v_{im}),\quad V^-=(\min_i v_{i1},\ldots,\min_i v_{im})
$$
$$
D_i^+=\sqrt{\sum_{j=1}^m (v_{ij}-V_j^+)^2},\quad D_i^-=\sqrt{\sum_{j=1}^m (v_{ij}-V_j^-)^2}
$$
相对贴近度仍为 $S_i=\dfrac{D_i^-}{D_i^+ + D_i^-}$。这就是更常用的**熵权-TOPSIS**或**AHP-TOPSIS**综合评价模型。
:::
